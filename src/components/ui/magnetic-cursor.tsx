...
export const MagneticCursor: FC<MagneticCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const onMouseMove = (event: PointerEvent) => {
      ...
      // Sadece wrapper içerisindeyken görünür yap
      const isInsideWrapper = wrapperRef.current && wrapperRef.current.contains(event.target as Node);

      gsap.to(cursorEl, { opacity: isInViewport && isInsideWrapper ? 1 : 0, duration: 0.2, overwrite: 'auto' });
  ...
  return (
    <div ref={wrapperRef} className="relative w-full h-full flex flex-col flex-1">
